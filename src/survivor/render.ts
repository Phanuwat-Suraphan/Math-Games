...
function drawBehaviorMark(
  ...
  if (enemy.behavior === 'healer') {
     ...
     */
    /*
     * วาดสองชั้น เส้นเข้มหนาก่อน แล้วเส้นสว่างบางทับ
     *
     * ชั้นเดียวสีเขียวอ่อนอ่านออกดีในทุ่งหญ้า แต่จางเกือบหมดในป่าลึก
     * ซึ่งพื้นเป็นสีเขียวเหมือนกัน เห็นได้จากการเรนเดอร์เทียบสองสนาม
     * สองชั้นอ่านออกทั้งบนพื้นสว่างและพื้นเข้ม
     * เป็นวิธีเดียวกับตัวหนังสือบนแถบเวลาของไอเทม
     */
    ctx.setLineDash([9, 7])
    ctx.strokeStyle = 'rgba(6,46,26,.45)'
    ctx.lineWidth = 4
    ctx.beginPath()
    ctx.arc(x, y, HEAL_RADIUS, 0, Math.PI * 2)
    ctx.stroke()
    ctx.strokeStyle = 'rgba(190,255,214,.95)'
    ctx.lineWidth = 2
    ctx.beginPath()
    ctx.arc(x, y, HEAL_RADIUS, 0, Math.PI * 2)
    ctx.stroke()
    ...
    /*
     * ไม่มีเครื่องหมายบนตัวแล้ว เพราะตัวมันเองมีกากบาทอยู่ในภาพ
     *
     * ตอนที่มันยังใช้ภาพร่วมกับผีสมการ กากบาทที่วาดทับคือสิ่งเดียวที่แยกสองตัวออก
     * พอมันมีภาพของตัวเองที่มีกากบาทอยู่แล้ว การวาดกากบาทซ้อนอีกอันคือการซ้ำ
     * ซึ่งกินที่บนจอโดยไม่ได้เพิ่มข้อมูลอะไรเลย
     */
    return
  }
